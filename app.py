    ...
    if request.method == 'POST' and form.validate():
        ...
        if result > 0:
            ...
            if sha256_crypt.verify(password_candidate, password):
                ...
                # message = Markup("<h1>Wecome" ,{{session.s_name}}) , "</h1>"
                # flash(message)
                flash('You are logged in', 'success')

                return redirect(url_for('index'))
                # flash("welcome" + {{session.s_name}})
                # flash('You are logged in', 'success')
...
class RegisterForm(Form):
    ...
    password = PasswordField('', [validators.length(min=3)],
                             render_kw={'placeholder': 'Password'})
    mobile = StringField('', [validators.length(min=3)], render_kw={'placeholder': 'Mobile'})
...
class UpdateRegisterForm(Form):
    name = StringField('Full Name', [validators.length(min=1)],
                       render_kw={'autofocus': True, 'placeholder': 'Full Name'})
    email = EmailField('Email', [validators.DataRequired(), validators.Email(), validators.length(min=1)],
                       render_kw={'placeholder': 'Email'})
